...
class ExchangeController extends Controller
{
    ...
    public function exchange(Request $request)
    {
        ...
        $receiveAmount = $request->reciving_amount;
        ...
        */
        ...
        if ($sendAmount < $sendCurrency->minimum_limit_for_sell) {
            $notify[] = ['error', "Minimum sending amount " . showAmount($sendCurrency->minimum_limit_for_sell) . ' ' . $sendCurrency->cur_sym];
            return back()->withNotify($notify)->withInput();
        }

        if ($sendAmount > $sendCurrency->maximum_limit_for_sell) {
            $notify[] = ['error', "Maximum sending amount " . showAmount($sendCurrency->maximum_limit_for_sell) . ' ' . $sendCurrency->cur_sym];
            return back()->withNotify($notify)->withInput();
        }

        if ($receiveAmount < $receiveCurrency->minimum_limit_for_buy) {
            $notify[] = ['error', "Minimum received amount " . showAmount($receiveCurrency->minimum_limit_for_buy) . ' ' . $receiveCurrency->cur_sym];
            return back()->withNotify($notify)->withInput();
        }

        if ($receiveAmount > $receiveCurrency->maximum_limit_for_buy) {
            $notify[] = ['error', "Maximum received amount " . showAmount($receiveCurrency->maximum_limit_for_buy) . ' ' . $receiveCurrency->cur_sym];
            return back()->withNotify($notify)->withInput();
        }
        /*
        ...
        */
        ...
        $exchange->charge = isset($charge) ? $charge : 0;
        $exchange->save();
